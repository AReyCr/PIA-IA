...
def DFS(i, T, val, weight):
    ...
    if weight + weights[i] <= capacity:
        T[i] = 1
        if value(T.copy()) <= max_val:
            T[i] = 0
        else:
            DFS(i+1, T.copy(), val + values[i], weight + weights[i])
    DFS(i+1,T.copy(),val,weight)
...
for i in range(10):
    for j in range(50):
        ...
        max_T = []
        max_val = 0
        DFS(0, start_state.copy(), 0, 0)

        end_time = time.time()

        total_time = end_time - start_time 

        print(i+1, j+1, value(max_T), total_time, best_value)
