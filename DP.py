...
def knapSack(W, wt, val, n, sol):
 
    #print("Cap:", W, "n:", n-1, "Weights:", wt, "Values:", val, "Solution:", sol)
    global highest_value
    global final_solution
    if n == 0 or W == 0:
        if total_value(sol, val) > highest_value:
            highest_value = total_value(sol, val)
            final_solution = sol
        return 0

    if (wt[n-1] > W):
        return knapSack(W, wt, val, n-1, sol)

    else:
        sol_s = sol.copy()
        sol_s[n-1] = 1
        return max(val[n-1] + knapSack(W-wt[n-1], wt, val, n-1, sol_s), knapSack(W, wt, val, n-1, sol))
...
def total_value(sol, vals):
    total = 0
    for i in range(len(sol)):
        total = total + (sol[i]* vals[i])
    return total
